...
def main(shapefile_path):
    ...
    for i, (exterior, holes) in enumerate(polygons):
        obj_name = f"Building_{i}"
        obj = create_mesh_from_polygon(obj_name, exterior, holes)
        print(f"Creato {obj_name} con {len(holes)} hole(s)")
        path = "/app/data/out.ply"
        export_mesh_ply(path, obj, True)
